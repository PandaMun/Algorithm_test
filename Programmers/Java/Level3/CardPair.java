...
public class CardPair {
    ...
    public static int solution(int[][] board, int r, int c) {
        ...
        // 현재 카드 종류를 isNum에 체크해놓고 체크해 둔 숫자 중에 순열로 뽑을 예정
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int l = 0; l < len; l++) {
                ...
                if (board[now.x][now.y] == arr[idx] && !boardVisited[now.x][now.y]) {
                    ...
                    // 지금까지 최소 이동 회수 더해주기
                    answerCnt += cnt;
                    // 이동 회수 초기화 -1인 이유는 밑에서 바로 +1을 해주면 0이 되기 때문
                    cnt = -1;
                    // 이미 방문한 말판임을 체크하기
                    boardVisited[now.x][now.y] = true;
                    // 큐 및 visited 비워주기(다시 최단 거리로 이동하기 위해)
                    q.clear();
                    ...
                    if (!isSecond) {
                        ...
                        // 다음은 새로운 첫 번째 말판을 찾아야 함을 체크
                        isSecond = false;
                        // 다음 말 종류를 찾기 위해 순열로 뽑은 배열의 index를 증가시켜줌
                        idx++;
